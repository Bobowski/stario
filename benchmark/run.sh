...
usage() {
  cat <<'EOF'
Usage: benchmark/run.sh [stario|fastapi|blacksheep-uvicorn|blacksheep-granian|sanic ...]
...
client=wrk
keep_raw=$KEEP_RAW
payload_file=benchmark/validate.lua
ports=$(port_list)
EOF
  ...
  [[ "$KEEP_RAW" == 1 ]] && echo "Raw output: ${RUN_DIR#$ROOT/}/*.txt"
}
